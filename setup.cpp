...
/* ==================================================================================================
   NACA 2412 - ensayo 2D con REFINAMIENTO GLORIA: la grilla fina (dx/2 por nivel) se anida alrededor
   del perfil, asi el dominio exterior puede ser grande y barato y el perfil queda a maxima resolucion.

   Con n_niveles = 1 este setup es identico al ensayo original (una sola grilla uniforme).
   Con n_niveles = 2 el perfil se resuelve con celdas de dx/2; con 3 niveles, dx/4; etc.
   La resolucion se elige ahora con "chord_cells" (celdas de cuerda EN EL NIVEL 0): la cuerda efectiva
   sobre el perfil es chord_cells * 2^(n_niveles-1).

   Ejemplo de presupuesto (box 16c x 10c):
       original uniforme, 264 celdas/cuerda  -> 11.2 M celdas
       GLORIA: 132 c/c nivel 0 + 1 parche    ->  3.1 M celdas y la MISMA resolucion en el perfil
       GLORIA: 132 c/c nivel 0 + 2 parches   ->  3.5 M celdas y 528 celdas/cuerda en el perfil

   EXTENSIONES REQUERIDAS en defines.hpp (ya configuradas):
       D2Q9, TRT, FP16C, FORCE_FIELD, EQUILIBRIUM_BOUNDARIES, SUBGRID, INTERACTIVE_GRAPHICS

   COMO SE USA: igual que el ensayo original (P para arrancar, Esc para salir). Los coeficientes se
   miden en el nivel MAS FINO. Al final se escriben los .vtk de TODOS los niveles con origen y
   espaciado SI correctos: en ParaView se abren juntos y quedan superpuestos en su lugar.
   El tutorial completo del metodo esta en TUTORIAL-GLORIA.md.
   ================================================================================================== */
void main_setup() {

	// ===== 1. PARAMETROS DEL ENSAYO (editar solo este bloque) =======================================
	const float si_u       = 50.0f;      // velocidad de corriente libre [m/s]
	const float si_chord   = 0.61f;      // cuerda del perfil [m]
	const float si_rho     = 1.225f;     // densidad del aire [kg/m^3]
	const float si_nu      = 9.8387E-6f; // viscosidad cinematica del aire [m^2/s]
	const float alpha_deg  = 5.0f;       // <<<<< ANGULO DE ATAQUE [grados]

	const uint  n_niveles  = 2u;         // <<<<< niveles GLORIA (1 = sin refinamiento, 2 = un parche a dx/2, 3 = dos parches anidados a dx/4)
	const uint  chord_cells= 132u;       // celdas de cuerda en el NIVEL 0 (cuerda efectiva en el perfil = chord_cells * 2^(n_niveles-1))
	const float ref_margin = 0.60f;      // margen del primer parche alrededor del perfil [cuerdas] (se reduce a la mitad por nivel)
	const float ref_wake   = 1.00f;      // extension extra del parche aguas abajo, para la estela cercana [cuerdas]

	const float n_conv     = 40.0f;      // duracion del ensayo en tiempos convectivos (en tiempo del NIVEL 0)
	const float prom_desde = 0.5f;       // desde que fraccion del ensayo se promedia
	const float box_x      = 16.0f;      // largo del dominio, en cuerdas
	const float box_y      = 10.0f;      // alto del dominio, en cuerdas
	const float x_le       = 4.0f;       // distancia entrada -> borde de ataque, en cuerdas
	const float lbm_u      = 0.075f;     // velocidad en unidades de red (identica en todos los niveles: escalado acustico)
	const uint  cada       = 100u;       // cada cuantos pasos del nivel 0 se mide Cl/Cd/Cm
	// ===============================================================================================

	// ===== 2. GRILLA, UNIDADES Y VISCOSIDAD ========================================================
	const float lbm_chord = (float)chord_cells;                     // cuerda en celdas del nivel 0
	const uint3 lbm_N = uint3(to_uint(box_x*lbm_chord), to_uint(box_y*lbm_chord), 1u);
	const float lbm_nu = lbm_u*lbm_chord/(si_u*si_chord/si_nu);     // nu del nivel 0 desde el Reynolds
	const float si_Re  = si_chord*si_u/si_nu;
	const float si_T   = n_conv*si_chord/si_u;                      // tiempo fisico del ensayo [s]

	GloriaLBM gloria(lbm_N, lbm_nu);

	// parches de refinamiento alrededor del perfil (coordenadas en celdas del nivel 0)
	const float cx0 = x_le*lbm_chord;                 // borde de ataque (x)
	const float cy0 = 0.5f*(float)lbm_N.y;            // eje del perfil (y)
	for(uint k=1u; k<n_niveles; k++) {
		const float mk = ref_margin*lbm_chord/(float)(1u<<(k-1u)); // margen del nivel k [celdas nivel 0]
		const float wk = ref_wake  *lbm_chord/(float)(1u<<(k-1u)); // extension de estela del nivel k
		const float hy = 0.12f*lbm_chord+mk;                       // semi-altura: semi-espesor proyectado del perfil + margen
		gloria.add_level(cx0-mk, cy0-hy, cx0+lbm_chord+mk+wk, cy0+hy);
	}
	gloria.set_si_units(lbm_chord, lbm_u, 1.0f, si_chord, si_u, si_rho);
	const ulong lbm_T = units.t(si_T);                              // pasos del ensayo (en pasos del NIVEL 0)

	const uint  Lf = gloria.get_levels()-1u;                        // indice del nivel mas fino
	const float chord_f = lbm_chord*(float)gloria.scale_of(Lf);     // cuerda en celdas del nivel fino
	const float tau0 = 3.0f*lbm_nu+0.5f;

	gloria.print_summary();
	print_info("=========================================================");
	print_info("NACA 2412 2D + GLORIA   |   AOA = "+to_string(alpha_deg, 2u)+" deg   |   "+to_string(gloria.get_levels())+" nivel(es)");
	print_info("Re      = "+to_string(to_uint(si_Re))+"   (c = "+to_string(si_chord, 3u)+" m, u = "+to_string(si_u, 1u)+" m/s)");
	print_info("Cuerda  = "+to_string(lbm_chord, 1u)+" celdas nivel 0  ->  "+to_string(chord_f, 1u)+" celdas efectivas en el perfil");
	print_info("Ensayo  = "+to_string(si_T, 4u)+" s = "+to_string(lbm_T)+" pasos del nivel 0 = "+to_string(n_conv, 1u)+" tiempos convectivos");
	print_info("=========================================================");
	if(tau0<0.502f) print_warning("tau nivel 0 = "+to_string(tau0, 6u)+" pegado a 0.5: la disipacion del dominio exterior la fija SUBGRID. En el nivel fino tau esta el doble de lejos de 0.5 por cada nivel: la capa limite se resuelve alli.");

	// ===== 3. GEOMETRIA: PERFIL FIJO + CORRIENTE LIBRE INCLINADA ===================================
	const float alpha = radians(alpha_deg);
	const float ca = cosf(alpha), sa = sinf(alpha);
	const float lbm_ux = lbm_u*ca, lbm_uy = lbm_u*sa;

	const float3 le      = float3(cx0, cy0, 0.0f);                       // borde de ataque (celdas nivel 0)
	const float3 center  = float3(le.x+0.50f*lbm_chord, le.y, 0.0f);     // centro del bounding box del perfil
	const float3 quarter0= float3(le.x+0.25f*lbm_chord, le.y, 0.0f);     // 1/4 de cuerda (celdas nivel 0, absolutas)

	gloria.voxelize_stl(get_exe_path()+"../stl/NACA2412-0.61-bin.stl", center, lbm_chord); // voxeliza en TODOS los niveles

	gloria.for_each_level([&](LBM& lbm, uint l) { // corriente libre inclinada en todos los niveles (misma u de red: escalado acustico)
		...
		parallel_for(lbm.get_N(), [&](ulong n) { uint x=0u, y=0u, z=0u; lbm.coordinates(n, x, y, z);
			...
			if(l==0u&&(x==0u||x==Nx-1u||y==0u||y==Ny-1u)) lbm.flags[n] = TYPE_E; // entrada/salida/paredes abiertas SOLO en el nivel 0
		});
	});

	ulong solid_f = 0ull; // control de voxelizacion en el nivel fino
	for(ulong n=0ull; n<gloria.finest().get_N(); n++) if(gloria.finest().flags[n]==TYPE_S) solid_f++;
	print_info("Celdas solidas del perfil en el nivel fino: "+to_string(solid_f));
	if(solid_f==0ull) print_error("El perfil no se voxelizo en el nivel fino. Revisa la ruta del .stl (STL BINARIO) y que el parche contenga al perfil.");

	// ===== 4. VISUALIZACION INTERACTIVA ============================================================
#ifdef GRAPHICS
	gloria.for_each_level([&](LBM& lbm, uint l) {
	...
	// ===== 5. BUCLE PRINCIPAL CON MUESTREO DE FUERZAS ==============================================
	// Coeficientes medidos en el NIVEL FINO, directo en unidades de red de ese nivel:
	// rho = 1 y la envergadura de la tajada 2D es 1 celda fina -> area de referencia = cuerda fina x 1.
	const float lbm_q = 0.5f*sq(lbm_u)*chord_f;
	const float f_Npm = gloria.units_of(Lf).si_F(1.0f)/gloria.units_of(Lf).si_x(1.0f); // fuerza de red fina -> N/m
	const float3 quarter_f = gloria.to_level(Lf, quarter0)-gloria.finest().center();   // 1/4 de cuerda en coords centradas del nivel fino

	vector<float> s_Cl, s_Cd, s_Cm;
	string csv = "# t[s]\tpaso_L0\tCl\tCd\tCm_c4\tL_por_m[N/m]\tD_por_m[N/m]\tmismatch_interfaz\n";

	gloria.run(0u); // configura interfaces e inicializa todos los niveles (la ventana arranca EN PAUSA)
#ifdef GRAPHICS
	gloria.show_level(Lf); // la ventana muestra el nivel fino; cambiar con gloria.show_level(0) para ver el dominio completo
#endif // GRAPHICS
	print_info("Listo. Apreta P en la ventana para arrancar la simulacion.");

	while(gloria.get_t()<lbm_T&&running) {
		gloria.run(min((ulong)cada, lbm_T-gloria.get_t()));
		if(!running) break;

		const float3 F = gloria.object_force(Lf);              // fuerza sobre el perfil (unidades de red del nivel fino)
		const float3 M = gloria.object_torque(Lf, quarter_f);  // momento respecto del 1/4 de cuerda
		const float drag =  F.x*ca+F.y*sa;
		const float lift = -F.x*sa+F.y*ca;
		const float Cl = lift/lbm_q;
		const float Cd = drag/lbm_q;
		const float Cm = -M.z/(lbm_q*chord_f);
		const float mm = gloria.get_levels()>1u ? gloria.interface_mismatch(1u)/lbm_u : 0.0f; // error relativo de acople en la interfaz

		s_Cl.push_back(Cl); s_Cd.push_back(Cd); s_Cm.push_back(Cm);
		csv += to_string(units.si_t(gloria.get_t()), 6u)+"\t"+to_string(gloria.get_t())+"\t"+to_string(Cl, 5u)+"\t"+to_string(Cd, 5u)+"\t"+to_string(Cm, 5u)+"\t"+to_string(lift*f_Npm, 3u)+"\t"+to_string(drag*f_Npm, 3u)+"\t"+to_string(mm, 5u)+"\n";
		print_info("t="+to_string(units.si_t(gloria.get_t()), 5u)+"s ("+to_string(100.0f*(float)gloria.get_t()/(float)lbm_T, 1u)+"%)   Cl="+to_string(Cl, 4u)+"   Cd="+to_string(Cd, 4u)+"   Cm="+to_string(Cm, 4u)+"   acople="+to_string(100.0f*mm, 2u)+"%");
	}

	// ===== 6. POST-PROCESADO =======================================================================
	const uint ns = (uint)s_Cl.size();
	if(ns==0u) print_error("No se tomo ninguna muestra: subi n_conv o baja 'cada'.");
	const uint i0raw = (uint)(prom_desde*(float)ns);
	const uint i0 = i0raw>=ns ? ns-1u : i0raw;
	const uint nm = ns-i0;

	float Cl_m=0.0f, Cd_m=0.0f, Cm_m=0.0f;
	for(uint i=i0; i<ns; i++) { Cl_m += s_Cl[i]; Cd_m += s_Cd[i]; Cm_m += s_Cm[i]; }
	Cl_m /= (float)nm; Cd_m /= (float)nm; Cm_m /= (float)nm;
	float Cl_sd=0.0f, Cd_sd=0.0f, Cm_sd=0.0f;
	for(uint i=i0; i<ns; i++) { Cl_sd += sq(s_Cl[i]-Cl_m); Cd_sd += sq(s_Cd[i]-Cd_m); Cm_sd += sq(s_Cm[i]-Cm_m); }
	Cl_sd = sqrtf(Cl_sd/(float)nm); Cd_sd = sqrtf(Cd_sd/(float)nm); Cm_sd = sqrtf(Cm_sd/(float)nm);

	const float q_si  = 0.5f*si_rho*sq(si_u)*si_chord;
	const float L_si  = Cl_m*q_si;
	const float D_si  = Cd_m*q_si;
	const float eff   = Cd_m!=0.0f ? Cl_m/Cd_m : 0.0f;

	const string tag  = "aoa"+replace(to_string(alpha_deg, 1u), ".", "p")+"-G"+to_string(gloria.get_levels());
	const string path = get_exe_path()+"export/";

	string res = "";
	res += "===============================================================\n";
	res += " NACA 2412 - FluidX3D-GLORIA (LBM D2Q9, TRT, SUBGRID/LES)\n";
	res += "===============================================================\n";
	res += "\n[ENSAYO]\n";
	res += "  angulo de ataque      = "+to_string(alpha_deg, 2u)+" deg\n";
	res += "  velocidad             = "+to_string(si_u, 2u)+" m/s\n";
	res += "  cuerda                = "+to_string(si_chord, 4u)+" m\n";
	res += "  Reynolds              = "+to_string(to_uint(si_Re))+"\n";
	res += "\n[GLORIA]\n";
	res += "  niveles               = "+to_string(gloria.get_levels())+"\n";
	res += "  cuerda nivel 0        = "+to_string(lbm_chord, 1u)+" celdas (dx = "+to_string(1000.0f*gloria.units_of(0u).si_x(1.0f), 3u)+" mm)\n";
	res += "  cuerda nivel fino     = "+to_string(chord_f, 1u)+" celdas (dx = "+to_string(1000.0f*gloria.units_of(Lf).si_x(1.0f), 3u)+" mm)\n";
	res += "  celdas solidas (fino) = "+to_string(solid_f)+"\n";
	res += "  u_lbm                 = "+to_string(lbm_u, 4u)+" (identica en todos los niveles)\n";
	res += "  pasos nivel 0         = "+to_string(gloria.get_t())+" de "+to_string(lbm_T)+"\n";
	res += "  tiempo fisico         = "+to_string(units.si_t(gloria.get_t()), 5u)+" s\n";
	res += "\n[PROMEDIADO]\n";
	res += "  muestras totales      = "+to_string(ns)+" (una cada "+to_string(cada)+" pasos del nivel 0)\n";
	res += "  muestras promediadas  = "+to_string(nm)+" (ultimo "+to_string(100.0f*(1.0f-prom_desde), 0u)+"% del ensayo)\n";
	res += "\n[COEFICIENTES]  (area de referencia = cuerda x 1 m de envergadura, medidos en el nivel fino)\n";
	res += "  Cl  = "+to_string(Cl_m, 5u)+"   +/- "+to_string(Cl_sd, 5u)+" (desvio)\n";
	res += "  Cd  = "+to_string(Cd_m, 5u)+"   +/- "+to_string(Cd_sd, 5u)+" (desvio)\n";
	res += "  Cm  = "+to_string(Cm_m, 5u)+"   +/- "+to_string(Cm_sd, 5u)+" (desvio, respecto de c/4)\n";
	res += "  L/D = "+to_string(eff, 3u)+"\n";
	res += "\n[FUERZAS POR METRO DE ENVERGADURA]\n";
	res += "  sustentacion L' = "+to_string(L_si, 2u)+" N/m\n";
	res += "  resistencia  D' = "+to_string(D_si, 2u)+" N/m\n";
	res += "\n[ARCHIVOS]\n";
	res += "  serie_"+tag+".csv     : Cl/Cd/Cm paso a paso (+ error de acople en la interfaz)\n";
	res += "  polar.csv             : una fila por corrida\n";
	res += "  gloria/*-L<n>-*.vtk   : campos de CADA nivel, con origen/espaciado SI para superponer en ParaView\n";

	write_file(path+"serie_"+tag+".csv", csv);
	write_file(path+"resumen_"+tag+".txt", res);

	const string polar = path+"polar.csv";
	{ std::ifstream test(polar); if(!test.good()) write_file(polar, "# alpha[deg]\tCl\tCd\tCm_c4\tL_D\tCl_sd\tCd_sd\tRe\tcuerda_fina[celdas]\tpasos_L0\tniveles\n"); }
	write_line(polar, to_string(alpha_deg, 2u)+"\t"+to_string(Cl_m, 5u)+"\t"+to_string(Cd_m, 5u)+"\t"+to_string(Cm_m, 5u)+"\t"+to_string(eff, 3u)+"\t"+to_string(Cl_sd, 5u)+"\t"+to_string(Cd_sd, 5u)+"\t"+to_string(to_uint(si_Re))+"\t"+to_string(chord_f, 1u)+"\t"+to_string(gloria.get_t())+"\t"+to_string(gloria.get_levels()));

	gloria.write_vtk_all(path+tag+"/"); // u, rho y flags de TODOS los niveles, superponibles en ParaView
#ifdef GRAPHICS
	gloria.finest().graphics.write_frame(path+tag+"/");
#endif // GRAPHICS

	print_info("===============================================================");
	print_info("RESULTADOS   AOA = "+to_string(alpha_deg, 2u)+" deg   Re = "+to_string(to_uint(si_Re))+"   GLORIA x"+to_string(gloria.get_levels()));
	print_info("  Cl  = "+to_string(Cl_m, 5u)+"  +/- "+to_string(Cl_sd, 5u));
	print_info("  Cd  = "+to_string(Cd_m, 5u)+"  +/- "+to_string(Cd_sd, 5u));
	print_info("  Cm  = "+to_string(Cm_m, 5u)+"  +/- "+to_string(Cm_sd, 5u)+"   (c/4)");
	print_info("  L/D = "+to_string(eff, 3u));
	print_info("  L' = "+to_string(L_si, 2u)+" N/m      D' = "+to_string(D_si, 2u)+" N/m");
	print_info("Post-procesado escrito en "+path);
	print_info("===============================================================");

#if defined(INTERACTIVE_GRAPHICS)||defined(INTERACTIVE_GRAPHICS_ASCII)
	print_info("Simulacion detenida. La ventana queda abierta (Esc para salir).");
	while(running) sleep(0.1);
#endif // INTERACTIVE_GRAPHICS
}


/*
void main_setup() { // VALIDACION GLORIA: cilindro 2D con calle de von Karman, Re=200; extensiones: D2Q9, TRT, FP16C, FORCE_FIELD, EQUILIBRIUM_BOUNDARIES, GRAPHICS o INTERACTIVE_GRAPHICS
	// Caso de validacion sin .stl: un cilindro de diametro D=16 celdas (nivel 0) centrado en un parche
	// refinado a dx/2. A Re=200 la calle de vortices es periodica y el St y el Cd son conocidos:
	//   referencia experimental/numerica: St ~ 0.196-0.20, Cd ~ 1.3-1.4
	// El mismo caso con n_niveles=1 y el doble de celdas sirve de patron de comparacion.
	const uint  n_niveles = 2u;
	const float lbm_u  = 0.05f;
	const float lbm_D  = 16.0f;              // diametro en celdas del nivel 0 (el fino lo ve con 32)
	const float Re     = 200.0f;
	const float lbm_nu = lbm_u*lbm_D/Re;
	const uint3 N      = uint3(32u*(uint)lbm_D, 16u*(uint)lbm_D, 1u); // 512 x 256
	GloriaLBM gloria(N, lbm_nu);
	const float cx = 8.0f*lbm_D, cy = 0.5f*(float)N.y;               // centro del cilindro (celdas nivel 0)
	for(uint k=1u; k<n_niveles; k++) {
		const float m = 2.0f*lbm_D/(float)(1u<<(k-1u));              // margen alrededor del cilindro
		gloria.add_level(cx-m, cy-m, cx+m+2.0f*lbm_D, cy+m);         // parche con extension de estela
	}
	gloria.for_each_level([&](LBM& lbm, uint l) {
		const float3 c = gloria.to_level(l, float3(cx, cy, 0.0f));   // centro del cilindro en celdas locales del nivel l
		const float  R = 0.5f*lbm_D*(float)gloria.scale_of(l);
		const uint Nx=lbm.get_Nx(), Ny=lbm.get_Ny();
		parallel_for(lbm.get_N(), [&](ulong n) { uint x=0u, y=0u, z=0u; lbm.coordinates(n, x, y, z);
			if(sq((float)x+0.5f-c.x)+sq((float)y+0.5f-c.y)<sq(R)) lbm.flags[n] = TYPE_S;
			else { lbm.u.x[n] = lbm_u; lbm.u.y[n] = 0.05f*lbm_u; } // la perturbacion en uy dispara el desprendimiento antes
			if(l==0u&&(x==0u||x==Nx-1u||y==0u||y==Ny-1u)) lbm.flags[n] = TYPE_E;
		});
	});
	gloria.print_summary();
	const uint Lf = gloria.get_levels()-1u;
	const float lbm_q = 0.5f*sq(lbm_u)*lbm_D*(float)gloria.scale_of(Lf); // presion dinamica x diametro (celdas finas)
	string csv = "# paso_L0\tCd\tCl\tmismatch\n";
	gloria.run(0u);
	while(gloria.get_t()<50000ull&&running) {
		gloria.run(100ull);
		const float3 F = gloria.object_force(Lf);
		const float mm = n_niveles>1u ? gloria.interface_mismatch(1u)/lbm_u : 0.0f;
		csv += to_string(gloria.get_t())+"\t"+to_string(F.x/lbm_q, 5u)+"\t"+to_string(F.y/lbm_q, 5u)+"\t"+to_string(mm, 5u)+"\n";
		print_info("t="+to_string(gloria.get_t())+"   Cd="+to_string(F.x/lbm_q, 4u)+"   Cl="+to_string(F.y/lbm_q, 4u)+"   acople="+to_string(100.0f*mm, 2u)+"%");
	}
	write_file(get_exe_path()+"export/validacion_cilindro_G"+to_string(n_niveles)+".csv", csv);
	gloria.write_vtk_all();
}
*/
...
void main_setup() { // Starship; required extensions in defines.hpp: FP16S, EQUILIBRIUM_BOUNDARIES, SUBGRID, INTERACTIVE_GRAPHICS or GRAPHICS
	// ################################################################## define simulation box size, viscosity and volume force ###################################################################
	const uint3 lbm_N = resolution(float3(1.0f, 2.0f, 2.0f), 6000u); // input: simulation box aspect ratio and VRAM occupation in MB, output: grid resolution
	const float lbm_Re = 10000000.0f;
	const float lbm_u = 0.05f;
	const ulong lbm_T = 108000ull;
	LBM lbm(lbm_N, 1u, 1u, 1u, units.nu_from_Re(lbm_Re, (float)lbm_N.x, lbm_u));
